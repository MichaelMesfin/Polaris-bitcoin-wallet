 ...
 */
package polaris.wallettemplate.walletfx.utils;
...
public class GuiUtils {
    ...
    public static void runAlert(BiConsumer<Stage, AlertWindowController> setup) {
        try {
            ...
            ///roundtrip/bitcoinj/walletfx/utils/alert.fxml
            
            
            
            FXMLLoader loader = new FXMLLoader(GuiUtils.class.getResource("/walletfx/utils/alert.fxml"));
    ...
    public static ScaleTransition zoomFit(Node node){
        
       return zoomFit(node,0);
    ...
    public static ScaleTransition zoomFit(Node node, int delayMills){
        
        return scaleFromTo(node, 0.8, 0.6,  delayMills);
    }

    public static ScaleTransition explodeOut(Node node) {
        return scaleFromTo(node, 0.8, 0.9, 0);
    }

    private static ScaleTransition scaleFromTo(Node node, double from, double to, int delayMillis) {
        ScaleTransition scale = new ScaleTransition(UI_ANIMATION_TIME.divide(4), node);
        scale.setFromX(from);
        scale.setFromY(from);
        scale.setToX(to);
        scale.setToY(to);
